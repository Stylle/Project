...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);//使能时钟GPIOA
	...
	GPIO_InitStructure.GPIO_Pin = LED_PIN;  //设置指定IO口
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  //设置速率
	GPIO_Init(LED_GPIO_PORT,&GPIO_InitStructure);//端口选择
	GPIO_SetBits(LED_GPIO_PORT,LED_PIN);//设置端口PB5默认为高电平
	//GPIO_ResetBits(GPIOB,GPIO_Pin_5);//设置led端口默认为低电平

...
void M7SEG_Port_Init()
{
	...
	GPIO_InitStructure.GPIO_Pin = M7SEG_PORT_PIN;  			//设置指定IO口
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  		//设置速率
	GPIO_Init(M7SEG_GPIO_PORT,&GPIO_InitStructure);			//端口选择
	GPIO_SetBits(M7SEG_GPIO_PORT,M7SEG_PORT_PIN);			//设置端口PB5默认为高电平
}
void M7SEG_Bit_Init()
{
	GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		//设置输出模式这里“推挽输出”
	GPIO_InitStructure.GPIO_Pin = M7SEG_BIT_PIN;  			//设置指定IO口
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  		//设置速率
	GPIO_Init(M7SEG_BIT_PORT,&GPIO_InitStructure);			//端口选择
	GPIO_SetBits(M7SEG_BIT_PORT,M7SEG_BIT_PIN);				//设置端口PB5默认为高电平
}
